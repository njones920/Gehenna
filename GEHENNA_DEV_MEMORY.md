...
### Spirit Conversation & 0.4.29 Release
- **Decision**: Bound spirits are conversable via `speak`. `ExpressionEngine.spiritChat` renders a `FullExpressionPacket` assembled by `spiritChatPacket`: tag-derived facts gated by the Knowledge attribute (4/7/12 caps at <0.3/<0.6/else), taboo-dimension tags as forbidden topics, root-identity true name, and per-epoch authored interiority. Every exchange costs `Retinue.strainPerExchange` (0.015) stability plus one command tick — conversation is a resource decision, per the milestone plan ("talking to the dead is holding a door open").
- **Epoch interiority**: `Epoch` gained optional interiority fields (same four-part model as NPCs). Authored for Hiram's three aspects and Maacah's three in `identities.json`. Verified in live play: asked about his sons, the Bronze Captain deflects into duty and lets "the count" falter without ever stating the wound — interiority shapes voice without leaking as fact. This is the quality bar for future epoch authoring.
- **Quality note**: routing `spiritSpeech` through the same root-identity-aware packet noticeably improved first-manifestation lines versus the tag-only packet. Any future spirit expression surface should pass the resolved `RootIdentity` when available.
- **Preserve**: knowledge gating is deterministic (stable fact order, no sampling). The LLM phrases; it does not choose what the spirit knows.

### The Relationship Ledger & 0.4.30 Release
- **Decision**: Added `World/SpiritRelationship.swift`: `RelationalMoment` (typed, valenced, append-only), `SpiritRelationship` (keyed by rootIdentityID so epoch aspects share one memory), `RelationshipLedger`, `FamiliarityStage`, and `RelationalInvocation`. Stage is a pure function of the moment record + personality traits. Valence table lives in `RelationalMoment.valence` — the single source of relational truth.
- **Pipeline**: `resolve(..., invocation:)` — call-by-name adds relationship-derived coherence (autopsy: "The knowing itself was an anchor"), confines root-identity matching to the called identity, and passes valence into `EpochResolver.scoreEpoch`, where soured relationships boost dark aspects (corruptionThreshold/hostile/butcher) and warm ones boost the rest. Verified live: banish the Bronze Captain once, call Hiram again, and the Ashkelon Butcher answers — angry, and it names the banishment ("it was theft — the ripping away without the proper rites").
- **CLI**: `call` command; "my name is X" in conversation records `.gaveTrueName` permanently; every departure and exchange writes to the ledger. Relationship facts enter chat packets ungated by Knowledge — the dead forget their lives, not their treatment.
- **Known tune item for 0.5.0**: spirit responses run long — numPredict/length validation needs tightening; multi-paragraph monologues dilute the register.
- **Preserve**: the valence table and stage thresholds are canon constants; changing them silently rewrites every existing relationship in every save. The ledger is engine truth — the LLM never writes to it.

### Typed Intent Extraction & 0.4.31 Release
- **Decision**: Free-form speech is classified into `ConversationalIntent` (closed enum) via deterministic word-boundary heuristics first, then a temperature-zero one-word LLM pass (`ExpressionProvider.classifyIntent`, a protocol *requirement* with nil default — extension-only methods statically dispatch to the default on existentials, which silently disabled classification until fixed). Engine applies all consequences: spirit moments via `intent.spiritMoment`, NPC deltas via `NPC.apply(_:)`. This resolves the "free-form chat is neutral" rule from 0.4.26 without violating Expression-renders-never-decides: the LLM transcribes player intent into the same typed space commands use.
...
### The Oracle Lane & 0.4.33 Release — Design Correction From Nate
- **Correction (2026-07-10)**: Nate's original vision is "Conway's Game of Life meets language compiler" — he wants non-determinism and the LLM genuinely driving story, and pushed back on the accumulating determinism emphasis. Resolution: the Codex's Oracle-Network recorded-envelope pattern applied to the LLM itself. **The model proposes; the simulation commits; the record remembers.**
- **Three lanes**: (1) spoken canon harvest — spirit improvisations extracted, recorded in `SpiritRelationship.spokenClaims`, annotated into the Codex, re-entering future packets so the dead stay self-consistent; (2) generative Director — `WorldEventProposal` (rumor/npcAction/omen/visitor) proposed by the LLM, gated by `ProposalValidator`, committed to the journal with deterministic effects, fired by live entropy after rituals; (3) live entropy in ritual seeds, recorded in journal tags (`seed:N`) as consumed.
